...
import { EmployeeEditStoreService } from '../store';

@Component({
  selector: 'app-employee-edit-container',
  standalone: true,
  imports: [CommonModule, MatCardModule, EmployeeFormComponent],
  providers: [provideComponentStore(EmployeeEditStoreService)],
  template: `
    <mat-card
      *ngIf="{
...
export class EmployeeEditContainerComponent implements CanLeave {
  ...
  private readonly store: EmployeeEditStoreService = inject(
    EmployeeEditStoreService
  );
  ...
  protected onFormSaved(formData: TaskFormData): void {
    this.store.saveEmployee(formData);
  }
}
